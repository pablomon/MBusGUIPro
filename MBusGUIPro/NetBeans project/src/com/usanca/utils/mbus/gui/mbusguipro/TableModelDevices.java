 ...
 */
public class TableModelDevices extends AbstractTableModel {
    
    private String[] columnNames = {"  ", "Primaria", "Secundaria", "Nombre asignado por el usuario"};
    private ArrayList<MBusDevice> devices = new ArrayList();   
    
    public TableModelDevices(ArrayList<MBusDevice> dl) {        
        this.devices = dl;
    ...
    public void setDevicesList(ArrayList<MBusDevice> dl) {
        if (dl != null) {
            devices = dl;            
            Printer.out("Lista cargada");
            fireTableDataChanged();
        } else {
            Printer.out("ERROR: La lista devuelve null");
        }
    }
